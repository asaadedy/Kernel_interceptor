...
asmlinkage long interceptor(struct pt_regs reg) {
	spin_lock(&pidlist_lock);																		// for synchronization						
    asmlinkage long (*temp_f)(struct pt_regs) = table[reg.ax].f;
    if(table[reg.ax].monitored == 2 && (check_pid_monitored((int)reg.ax, current->pid) == 0)){		//if all pid's are monitered and pid is not in black list
        log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
    } else if((check_pid_monitored((int)reg.ax, current->pid) == 1) && table[reg.ax].monitored == 1){//if pid is in the monitered list 
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
    }
    spin_unlock(&pidlist_lock);																		// release the lock		
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	//check whether the syscal is valid number
	if (syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL){
		return -EINVAL; 
	}
	if (cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING){
		// if pid is not 0, and invalid or less than zero.
		if (current_uid() != 0){
            if (pid == 0){
                return -EPERM;
            }
            if ((check_pid_from_list(current->pid, pid) != 0)){					//chekc if pid is a root pid
                return -EPERM;
            }
        }
        if (pid < 0){
		...
		if (cmd == REQUEST_START_MONITORING ){
			spin_lock(&pidlist_lock);
            if (pid != 0){														//if pid is 0 then moniter all pid's by changing moniterd = 2 , otherwise monitered = 1 
                if (table[syscall].intercepted == 0){							//if the syscall is not intercepted
				    spin_unlock(&pidlist_lock);
                    return -EINVAL;
			    }
                if (check_pid_monitored(syscall, pid) == 1){					//if pid is already  monitered
                    spin_unlock(&pidlist_lock);
                    return -EBUSY;
                }
			    if (-ENOMEM  == add_pid_sysc( pid, syscall)){					//adding the pid to the monitered list
				    spin_unlock(&pidlist_lock);
                    return -ENOMEM;
			    }
		...
		if (cmd == REQUEST_STOP_MONITORING ){
            ...
            }else {																//if monitered = 2 then add the pid to the black_list	
                if (table[syscall].intercepted == 0){							//if syscall is not intercepted
                    spin_unlock(&pidlist_lock);
                    return -EINVAL;
                }
                if (check_pid_monitored(syscall, pid) == 1){					//if pid is already in the black list
                    spin_unlock(&pidlist_lock);
                    return -EBUSY;
                }
                if (-ENOMEM == add_pid_sysc(pid, syscall)){						//add the pid to the black list
                    spin_unlock(&pidlist_lock);
                    return -ENOMEM;
                }
            }
	...
	if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){ 
		...
		if (cmd == REQUEST_SYSCALL_INTERCEPT){
            ...
            if (table[syscall].intercepted == 1){
                spin_unlock(&pidlist_lock);
                return -EBUSY;
			}												// set the sys_call_table to RW
            spin_lock(&calltable_lock);
			set_addr_rw((unsigned long) sys_call_table);	//save the original system call at index MY_CUSTOM_SYSCALL
            table[syscall].f = sys_call_table[syscall];
			sys_call_table[syscall] = interceptor;
            table[syscall].intercepted = 1;
			table[syscall].monitored = 0;
			table[syscall].listcount = 0;
			INIT_LIST_HEAD(&(table[syscall].my_list));		// set the sys_call_table to Read only
		...
		if (cmd == REQUEST_SYSCALL_RELEASE){
            ...
            spin_lock(&calltable_lock);
			set_addr_rw((unsigned long) sys_call_table);				//save the original system call at index MY_CUSTOM_SYSCALL back to the sys_call_table
            sys_call_table[syscall] = table[syscall].f;
			table[syscall].intercepted = 0;
            destroy_list(syscall);										// set the sys_call_table to Read only
			set_addr_ro((unsigned long) sys_call_table);
		    spin_unlock(&calltable_lock);
            spin_unlock(&pidlist_lock);
		}
...
/**
 * place holder for system original functions
 */
...
static int init_function(void) {
	...
	int i = 0;												// set the sys_call_table to RW
    spin_lock(&calltable_lock);
	set_addr_rw((unsigned long) sys_call_table);			//save the original system call at index MY_CUSTOM_SYSCALL
	orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];//intercept the original call at index MY_CUSTOM_SYSCAL with my_syscall
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall;			//save the original system call at index __NR_exit_group
	orig_exit_group = sys_call_table[__NR_exit_group];		//intercept the original call at index __NR_exit_group with my_syscall
	sys_call_table[__NR_exit_group] = my_exit_group;	    //initialize every the my_list in every struct of syscall
    for (i = 1; i <= NR_syscalls; i++){
		INIT_LIST_HEAD(&(table[i].my_list));
    }														// set the sys_call_table to Read only
    set_addr_ro((unsigned long) sys_call_table);
    spin_unlock(&calltable_lock);
...
static void exit_function(void){        
	...
	spin_lock(&calltable_lock);
    set_addr_rw((unsigned long) sys_call_table);			//save the original system call at index MY_CUSTOM_SYSCALL back to the sys_call_table
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;//save the original system call at index __NR_exit_group back to the sys_call_table
	sys_call_table[__NR_exit_group] = orig_exit_group;		// set the sys_call_table to Read only
	set_addr_ro((unsigned long) sys_call_table);
    spin_unlock(&calltable_lock);
}
